...
INPUT_VCF = "Results/raw_ID_HC.vcf"
...
PROGRESS_UPDATE_INTERVAL = 75000
...
def process_vcf(input_vcf, results_dir, summary_dir, wt_idx,
    ...
    for t_idx in treated_samples:
        ...
        output_vcf = os.path.join(results_dir, f"{sample_clean}_CS.vcf")
        summary_file = os.path.join(summary_dir, f"{sample_clean}_CS_summary.txt")
        ...
        with open(output_vcf, "w") as out:
            ...
            for i, line in enumerate(variant_lines, start=1):
                ...
                gts = [normalize_gt(parse_genotype(f)) for f in parts[9:]]
                ...
                # Collect non-SOI genotypes, ignoring missing calls
                background_gts = [gt for j, gt in enumerate(gts) if j != t_idx]
                background_gts = [gt for gt in background_gts if gt != "./."]

                if len(background_gts) == 0:
                    discarded += 1
                    continue

                count_00 = background_gts.count("0/0")
                count_11 = background_gts.count("1/1")

                max_hom = max(count_00, count_11)
                proportion = max_hom / len(background_gts)
    ...
    global_summary_file = os.path.join(summary_dir, "CS_global_summary.txt")

    percent_values = [e["percent_kept"] for e in global_summary_data]
    kept_values = [e["kept"] for e in global_summary_data]
    ...
    with open(global_summary_file, "w") as g:
        g.write("Indel Comparison Script - Global Summary\n")
        g.write("=" * 60 + "\n\n")
